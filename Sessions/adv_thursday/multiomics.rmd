...
The dataset is named  and is in the same data_directory.
...
This is also achievable with the `FRiP` function, which doesn't need the cellranger metadata file.
...
2.1 Calculate the nucleosome signal, transcription start site enrichment score, and the fraction of reads in peaks.

```{r activity-2}

```

2.2 Plot the different QC metrics, and set cutoffs for the ATAC + RNA data.

2.3 Subset the seurat object by the above QC cutoffs and process the object for both the RNA and ATAC assays.

### Subsample down to just RNA_cluster 0 and 1 to look at this subset
...
#finding differentially accissible peaks between atac cluster 0 and 1
diff_peaks <-
    FindMarkers(small_obj,
                assay = "ATAC",
                ident.1 = "0",
...
# Here, RNA clusters 1 and 2 are roughly equal to ATAC clusters 0 and 2
de_genes <-
    FindMarkers(small_obj,
                assay = "RNA",
                ident.1 = "1",
...
small_obj <-
    LinkPeaks(object = small_obj,
              ...
              min.distance = 5000,
              genes.use = rownames(de_genes)[1:200])
...
3.1 Subset ATAC clusters 0 and 1 from our object (or should I do cell types?) and find the top 5 differentially accessibile peaks between the two groups as determined by log-fold change

3.2 Create a feature plot of these regions

3.3 Identify what genes are associated with these peaks

3.4 Find what motifs are enriched in our differentially accessible peaks.
