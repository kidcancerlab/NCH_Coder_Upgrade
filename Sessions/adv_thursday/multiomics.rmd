---
title: "Coder Upgrade 2026: Multiomics analysis"
...
```{r}
data_path <- "workshop_data/rstudio-workshop/data/raw/"
session_path <- "Sessions/adv_thursday/"

h5_data <-
    Read10X_h5(paste0(
        data_path,
        "mouse_multiome/5k_Mouse_Kidney_MO_gemx_filtered_feature_bc_matrix.h5"
    ))
...
system(paste0(
  "tar -xvf ",
  data_path,
  "mouse_multiome/5k_Mouse_Kidney_MO_gemx_cell_types.tar.gz -C ",
  data_path,
  "mouse_multiome/"
  ))


cell_types <- read.delim(
  paste0(data_path, "mouse_multiome/cell_types/10x_Cloud/cell_types.csv"),
  sep = ",",
) %>% 
  column_to_rownames("barcode")

# add cell types to object
seurat_obj$coarse_cell_type <- cell_types[colnames(seurat_obj), ]$coarse_cell_type
seurat_obj$fine_cell_type <- cell_types[colnames(seurat_obj), ]$fine_cell_type
```
...
frag_file <- paste0(
        data_path,
        "mouse_multiome/5k_Mouse_Kidney_MO_gemx_atac_fragments.tsv.gz"
    )
...
metadata <-
    read.table(
        file = paste0(
            data_path,
            "mouse_multiome/5k_Mouse_Kidney_MO_gemx_per_barcode_metrics.csv"
        ),
        sep = ",",
        header = TRUE,
        row.names = NULL
...
annotations <- qs2::qs_read("workshop_data/rstudio-workshop/data/rdata/annotations.qs2")
...
cutoffs_atac <-
    tribble(
        ...
        "nCount_ATAC",        1000,     100000,
        "nucleosome_signal",  0,        0.75,
        "TSS.enrichment",     0.7,      2, 
        "ffip",               0.2,      1,
        "nFeature_ATAC",      500,      15000
    )
feature_hist(
    seurat_obj,
    features = c(
...
DimPlot(seurat_obj,
    group.by = c("ATAC_cluster", "coarse_cell_type"),
    reduction = "umap_atac",
    label = TRUE,
    repel = TRUE
) /
DimPlot(seurat_obj,
    group.by = c("RNA_cluster", "coarse_cell_type"),
    reduction = "umap_rna",
    label = TRUE,
    repel = TRUE
)
...
Run through the QC steps in the next chunk so that we are all working with the same object.

```{r}
my_seurat_obj <- NucleosomeSignal(my_seurat_obj)
my_seurat_obj <- TSSEnrichment(my_seurat_obj)


total_frags <-
      CountFragments(my_frag_file) %>%
      dplyr::filter(CB %in% colnames(my_seurat_obj)) %>% # filter out barcodes not in our object #nolint
      arrange(match(CB, colnames(my_seurat_obj))) %>% # arrange in same order as barcodes in object #nolint
      mutate(num_insertions = frequency_count * 2) %>% # make column for total fragments
      column_to_rownames("CB")

my_seurat_obj <- AddMetaData(my_seurat_obj, total_frags)
...
feature_hist(
  ...
  cutoff_table = cutoffs_both
)

DefaultAssay(my_seurat_obj) <- "RNA"
my_seurat_obj <- subset(
        my_seurat_obj,
        subset =
            nCount_RNA     >= cutoffs_both$min_val[1] &
            nCount_RNA     <= cutoffs_both$max_val[1] &
            percent_mt_rna <= cutoffs_both$max_val[2] &
            nCount_ATAC    >= cutoffs_both$min_val[3] &
            nCount_ATAC    <= cutoffs_both$max_val[3] &
            nucleosome_signal <= cutoffs_both$max_val[4] &
            TSS.enrichment    >= cutoffs_both$min_val[5] &
            FRiP              >= cutoffs_both$min_val[6] &
            FRiP              <= cutoffs_both$max_val[6]
    ) %>%
    NormalizeData(verbose = FALSE) %>%
    FindVariableFeatures(verbose = FALSE) %>%
    ScaleData(verbose = FALSE) %>%
...
my_seurat_obj$rna_clusters <- my_seurat_obj$seurat_clusters

rm(total_frags)
gc()
```

Now take this QC'ed object and process the ATAC seq data and perform some exploratory analysis.
...
Subset your object to just ATAC clusters 1 and 2 and re-process. Visualize your new clusters
...
Now that our object is QC'ed and processed, we can proceed with more in-depth analysis of our multiomic data. This will involve identifying differentially accessible peaks between cell types, and then connecting these peaks to our transcriptional data to uncover biological meaning.

## Subset down to just Epithelial Cells

For this analysis, we will just be comparing two epithelial cell types.

After subsetting down to our epithelial cells, we need to re-process our data, because this subset of cells will have a different set of computationally-defined clusters, for both the ATAC and RNA assays.
...
small_obj <-
    subset(seurat_obj,
        coarse_cell_type == "epithelial cell",
    ) %>%
    RunTFIDF() %>%
    FindTopFeatures(min.cutoff = "q0") %>%
    RunSVD() %>%
...
Now we'll find differentially accessible peaks. We will compare epithelial cells of the proximal tubule with those of the distal convoluted tubule.

```{r da_peaks}
DefaultAssay(small_obj) <- "ATAC"
Idents(small_obj) <- small_obj$fine_cell_type
da_peaks <-
    FindMarkers(small_obj,
        ident.1 = "epithelial cell of proximal tubule",
        ident.2 = "kidney distal convoluted tubule epithelial cell"
    ) %>%
...
rna_umap <-
    ...
    as_tibble() %>%
    mutate(fine_cell_type = as.vector(small_obj$fine_cell_type)) %>%
    dplyr::rename(
        UMAP_1 = umaprna_1,
        UMAP_2 = umaprna_2
    )
...
atac_umap <-
    ...
    full_join(rna_umap %>%
        dplyr::select(cell, fine_cell_type))
...
draw_cluster_1 <- "epithelial cell of proximal tubule"
draw_cluster_2 <- "kidney distal convoluted tubule epithelial cell"

# Make plot of the combined UMAP projections

subset(combined_umap, fine_cell_type %in% c(draw_cluster_1, draw_cluster_2)) %>% 
ggplot(
    aes(
        x = UMAP_1,
        y = UMAP_2
    ...
    geom_line(
        data = combined_umap %>%
            dplyr::filter(fine_cell_type == draw_cluster_1),
        aes(
            x = UMAP_1,
            y = UMAP_2,
            group = cell
    ...
    geom_line(
        data = combined_umap %>%
            dplyr::filter(fine_cell_type == draw_cluster_2),
        aes(
            x = UMAP_1,
            y = UMAP_2,
            group = cell
...
joint_umap / DimPlot(small_obj, reduction = "joint_umap", group.by = "fine_cell_type")
```

### Correlation of ATAC peaks with gene expression
```{r}
DefaultAssay(small_obj) <- "ATAC"
Idents(small_obj) <- "fine_cell_type"
...
# finding differentially accessible peaks between our cell types of interest
diff_peaks <-
    FindMarkers(
        small_obj,
        assay = "ATAC",
...
qs2::qs_save(
    diff_peaks,
    "Sessions/adv_thursday/diff_peaks.qs2"
)
if (all(rownames(small_obj) == diff_peaks$region)) {
    small_obj[["ATAC"]][["sig_peaks"]] <- diff_peaks$p_val_adj < 0.05
} else {
...
# Many of these peaks associated with genes are also differentially accessible between cell types

CoveragePlot(
    object = small_obj,
    region = links$gene[20],
    features = links$gene[20],
    idents = c("epithelial cell of proximal tubule", "kidney distal convoluted tubule epithelial cell"),
...
Recall that the features we are calculating motif enrichment within are differentially accessible between cell types.
...
qs2::qs_save(freq_matrix, "workshop_data/rstudio-workshop/data/rdata/freq_matrix.qs2")
# This takes a while, so lets read in the pre-made object

freq_matrix <- qs2::qs_read("workshop_data/rstudio-workshop/data/rdata/freq_matrix.qs2")
...
qs2::qs_save(small_obj, "workshop_data/rstudio-workshop/data/rdata/multiomics_small_obj_motifs.qs2")
# This takes a while, so lets read in the pre-made object

small_obj <-
    qs2::qs_read("workshop_data/rstudio-workshop/data/rdata/multiomics_small_obj_motifs.qs2")
