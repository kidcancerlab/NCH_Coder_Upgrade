...
```{r}
westernQuant <- read.table("Sessions/beginner_monday/westernQuant.txt",
                           header = TRUE)
...
The functions max, min, mean, and median will return some summary statistics. These functions -- along with many others -- won't return the expected result if there are NA values present. Use the argument `na.rm = TRUE` to get around this issue.
...
Use the `head()` function to get a quick first-glance at the dataset. 
