...
Talk to Amy Gross, Meren Cam and Jason Navarro
...
1. What is my name? Matthew Vesuvius Cannon
2. What is my quest? To vanquish lines greater than 80 characters
3. What is the airspeed velocity of an unladen swallow? 42
4. T/F: for 10X multiomics data, both the RNA and ATAC data have identical read lengths FALSE
5. How many reads per cell should you target for 10X multiome ATAC data? 100k
6. T/F: for 10X multiome ATAC data, we count number of peaks per gene to calculate gene activity TRUE
...
1. Why will a UMAP performed using both RNA and ATAC data be more informative than a UMAP performed using only one dataset? Increased dimensionality
2. Why might the cells from a cluster in one of the datasets be randomly scattered in the other dataset? Microenvironment/convergence
3. Why might we be interested in an ATAC peak that is correlated with the expression of a gene but is not located in the promoter region of (or even close to) that gene? enhancer/superenhancer
