...
So this is a bit of confusing code. You might expect that we're binarizing the expression matrix and transforming it into either zeros or ones, but we're binarizing the regulon activity of each cell and then calculating the proportion of cell that have that regulon active.

One goofy thing about this function, your scenicOptions file *has* to be named scenicOptions or it doesn't work.
```{r binarize}
...
```{r top_regulators}
top_regulators <- pivot_longer(regulon_activity_cell_type_scaled)
colnames(top_regulators) <- c("Regulon", "CellType", "RelativeActivity")
top_regulators <- top_regulators[which(top_regulators$RelativeActivity > 0.5), ]
