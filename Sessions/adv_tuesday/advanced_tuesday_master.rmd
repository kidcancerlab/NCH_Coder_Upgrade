...
```{r load_old_data}
old_b1 <-
  Load10X_Spatial(data.dir = paste0(data_path, "/spatial/GSM5773458_Old_mouse_brain_B1-2/"),
                  slice = "Old_B1")
old_b1$age <- "old"
old_b1$slice <- "B"

old_b1@images$Old_B1@scale.factors$lowres <- 
  old_b1@images$Old_B1@scale.factors$hires
```
...
old_b1[["percent.mt"]] <-
  PercentageFeatureSet(object = old_b1, 
                       pattern = "^Mt\\d")
...
SpatialFeaturePlot(object = old_b1,
                   features = c("nCount_Spatial",
                                "nFeature_Spatial",
                                "percent.mt"))
```
...
```{r old_normalization}
old_b1 <-
  SCTransform(object = old_b1,
              assay = "Spatial",
              vars.to.regress = "percent.mt",
              return.only.var.genes = FALSE)
old_b1_clustered <-
  RunPCA(object = old_b1,
         npcs = 30) %>%
  RunUMAP(dims = 1:30) %>%
  FindNeighbors(k.parm = 30) %>%
  FindClusters(resolution = 0.5)

DimPlot(object = old_b1_clustered,
        cols = colors)

SpatialDimPlot(object = old_b1_clustered,
               cols = colors)
```
