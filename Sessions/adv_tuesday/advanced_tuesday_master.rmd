...
```{r load_male_data}
W3_M_Kidney <-
  Load10X_Spatial(data.dir = "~/workshop_data/raw/GSE252772/GSM8704840_20201201-NMK-M-Fc1U1Z1B1/",
                  slice = "W3_M_Kidney")
W3_M_Kidney$age <- "w3"
...
SpatialFeaturePlot(object = W3_M_Kidney,
                   features = c("nCount_Spatial",
                                "nFeature_Spatial",
                                "percent.mt"),
                   pt.size.factor = 3000,
...
DimPlot(object = W3_M_Kidney_clustered,
        cols = colors)

SpatialDimPlot(object = W3_M_Kidney_clustered,
               cols = colors,
               pt.size.factor = 3000,
               image.alpha = 0)
```
...
ref_cell_type_full <- ref$cell_type
all_cell_types <- unique(ref$cell_type)
...
# qs_save(rctd_obj, "~/workshop_data/rdata/rctd_obj.qs2")
...
SpatialFeaturePlot(object = spatial,
                   features = "fibroblast",
               pt.size.factor = 3000,
               image.alpha = 0)
```
...
spatial@meta.data %>%
  dplyr::select(sex, all_cell_types) %>%
...
celltype_colors <-
  colorset[1:length(all_cell_types)]
names(celltype_colors) <- 
  levels(ref_cell_type_full)

scatterpies <- list()
for (sex in names(split)){
  spe <- split[[sex]]
  mat <- 
    spe@meta.data[, all_cell_types]
...
deconv_mat <-
  spatial@meta.data %>%
  dplyr::select(seurat_clusters, all_cell_types) %>%
