...
Use the `subset()` function to sub-sample the sobj object down to those cells where nonsense == "A" - from metadata we added at line 567 (*corrected*)
```{r}
nonsense <- subset(pre_sobj, nonsense == "A")
```

Use the `subset()` function to sub-sample your seurat object down to the 50 cells with the highest expression of the gene "Sat1" (*corrected*)
```{r}
# Option 1 - step-by-step walk-through (beginner option)
## Get Sat1 expression for each cell
sat1_expression <- FetchData(pre_sobj, vars = "Sat1")
# Add cell names as a column (easier to see what's happening)
## note: rownames are the cell ids
sat1_expression$cell <- rownames(sat1_expression)
## Sort cells by Sat1 expression (highest first)
sat1_decr <- sat1_expression[order(sat1_expression$Sat1, decreasing = TRUE), ]
## Take the top 50 cells
top_cells <- sat1_decr$cell[1:50]
## Subset the Seurat object
hi_sat1 <- subset(pre_sobj, cells = top_cells)

# Other (shorter, but similar) Options:
# Option 2:
## same as above, but one line for ordering and selecting top 50
## Order cells by expression (descending) and select top 50
top_cells2 <- rownames(sat1_expression)[order(-sat1_expression$Sat1)][1:50]
# Subset the Seurat object based on cell ids
hi_sat1_v2 <- subset(pre_sobj, cells = top_cells2)

## Option 3
## Use GetAssayData and select only values for Sat1
## (rows are features and columns are cell ids)
sat1_vals <- GetAssayData(pre_sobj, layer = "data")["Sat1", ]
hi_sat1_v3 <- subset(pre_sobj,
                     cells = names(sort(sat1_vals, decreasing = TRUE))[1:50]
)
```
