---
title: "Coder Upgrade 2026: Seurat Under the Hood"
...
```{r data_check}
if (dir.exists("~/workshop_data/raw/GSE232482/GSM7337030_mouse_scRNA-seq_P0_F1/filtered_feature_bc_matrix")) {
  message("The raw data has been downloaded and is available...")
  sobj <- Read10X("~/workshop_data/raw/GSE232482/GSM7337030_mouse_scRNA-seq_P0_F1/filtered_feature_bc_matrix") %>%
  ...
  if (class(sobj) == "Seurat") {
    ...
    rm(sobj)
    if(file.exists("~/workshop_data/rdata/f1_sobj_processed_annotated.qs2")) {
      pre_sobj <- qs_read("~/workshop_data/rdata/f1_sobj_processed_annotated.qs2")
...
- Objects are intended to be purpose-built containers that organize your data
  and facilitate proper interactions with it
...
- NOTE: in R, methods are sub-defined functions (distinct from other 
  languages)
...
# Specifically look at rownames (cell ids)
head(rownames(meta))
```
...
```{r}
example <- Read10X("~/workshop_data/raw/GSE232482/GSM7337030_mouse_scRNA-seq_P0_F1/filtered_feature_bc_matrix") %>%
  CreateSeuratObject(min.cells = 3, min.features = 200)
```
...
```{r}
sub_obj <- pre_sobj[sample(rownames(pre_sobj), 100),
                    sample(colnames(pre_sobj), 50)]
```
...
```{r}
Idents(pre_sobj) <- "anchor_cell_types"
table(Idents(pre_sobj))

Idents(pre_sobj) <- "this_is_not_a_real_column_in_the_data"
table(Idents(pre_sobj))

Idents(pre_sobj) <- pre_sobj$anchor_cell_types
table(Idents(pre_sobj))
...
Pause and reflect: What do you notice is different in the three representations
of this data?
- Normalization assumes that each cell should have similar sequencing depth. It
  makes total counts comparable between cells, then log-transforms to make both
  high- and low-expressing transcripts detectable
- Scaling assumes that each "feature" (gene) is of similar importance. It sets
  mean expression for each feature at 0 and divides by the SD to create a
  "Z-score" like expression metric
- SCTransform uses statistical modeling to accomplish both tasks in an operation
  that looks at variation relative to a predicted mean
...
```{r}
rm(pre_sobj_borked, raw_counts, bad_normalization)
gc()
```
...
```{r}
Idents(pre_sobj) <- "seurat_clusters"
...
fgfr_pos <- subset(pre_sobj, subset = Fgfr1 > 1)
```

```{r}
rm(just_0, cells_0, fgfr_os)
gc()
```
...
```{r}
rm(umap_data, pca_data)
gc()
```

## Get cell names for specific cells
```{r}
high_col <- WhichCells(pre_sobj, expression = Col1a1 > 2)
head(high_col)

fibroblast <- Cells(subset(pre_sobj, subset = anchor_cell_types == "fibroblast"))
length(fibroblast)
```
...
high_fgfr_data <-
  FetchData(pre_sobj,
    ...
    cells = Cells(fgfr_pos)
  )

high_fgfr_data[1:5, 1:5]

# Plot the percentage of fibroblasts in each sample
FetchData(pre_sobj,
    vars = c("orig.ident", "anchor_cell_types"),
    cells = Cells(fgfr_pos)
  ) %>%
  group_by(orig.ident) %>%
  summarize(pct_fib = sum(anchor_cell_types == "fibroblast") / n() * 100) %>%
  ggplot(aes(orig.ident, pct_fib)) +
    geom_col() +
...
Newer versions of Seurat implement the BPCells procedure for on-disk processing
...
```{r}
raw_matrix <- Read10X("~/workshop_data/raw/GSE232482/GSM7337030_mouse_scRNA-seq_P0_F1/filtered_feature_bc_matrix")
...
```{r}
# Option 1: 
rna_counts <- LayerData(pre_sobj, layer = "counts") |>
  as_tibble()

# Option 2 (walk-through version):
rna_counts <- LayerData(pre_sobj, layer = "counts")
# get top 6 rows (and first 5 columns for easy viewing)
rna_counts[1:6, 1:5]
# turn periods into zeros (could also convert to tibble like above)
rna_counts[1:6, 1:5] %>% as.matrix()
```

Use a Seurat function to make a vector that contains the names of the cells in the object
```{r}
cell_names <- Cells(pre_sobj)
# to view:
head(sobj_cells)
```
...
# also
length(Cells(pre_sobj))
```

Use the `subset()` function to sub-sample the sobj object down to those cells where predicted.id == "macrophage" (*corrected*)
```{r}
# table(pre_sobj$predicted.id)
macs <- subset(pre_sobj, predicted.id == "macrophage")
macs
```

Use the `subset()` function to sub-sample your seurat object down to the 50 cells with the highest expression of the gene "Runx1" (*corrected*)
```{r}
# Option 1: 
runx_express <- FetchData(pre_sobj, vars = "Runx1") |>
  slice_max(Runx1, n = 50) |>
  rownames()

high_runx1 <- subset(pre_sobj, cells = runx_express)
high_runx1

# Option 2 - step-by-step walk-through
## Get Runx1 expression for each cell
runx1_expression <- FetchData(pre_sobj, vars = "Runx1")
# Add cell names as a column (easier to see what's happening)
## note: rownames are the cell ids
runx1_expression$cell <- rownames(runx1_expression)
## Sort cells by runx1 expression (highest first)
runx1_decr <- runx1_expression[order(runx1_expression$Runx1, decreasing = TRUE), ]
## Take the top 50 cells
top_cells <- runx1_decr$cell[1:50]
## Subset the Seurat object
hi_runx1 <- subset(pre_sobj, cells = top_cells)

# Option 3:
## same as above, but one line for ordering and selecting top 50
## Order cells by expression (descending) and select top 50
top_cells2 <- rownames(runx1_expression)[order(-runx1_expression$Runx1)][1:50]
# Subset the Seurat object based on cell ids
hi_runx1_v2 <- subset(pre_sobj, cells = top_cells2)

## Option 4:
## Use GetAssayData and select only values for Runx1
## (rows are features and columns are cell ids)
runx1_vals <- GetAssayData(pre_sobj, layer = "data")["Runx1", ]
hi_runx1_v3 <- subset(pre_sobj,
                     cells = names(sort(runx1_vals, decreasing = TRUE))[1:50]
)
```
...
```{r}
random_features <- rownames(pre_sobj[sample(VariableFeatures(pre_sobj), 10), ])

random_normed <- LayerData(pre_sobj[random_features, ], layer = "data") |>
  as_tibble()

random_scaled <- LayerData(pre_sobj[random_features, ], layer = "scale.data") |>
  as_tibble()

ranges <- lapply(1:length(random_features), function(i) {
  feature <- random_features[i]
  range_normed <- range(random_normed[i, ]) |>
    as.character()
  range_scaled <- range(random_scaled[i, ])
  return(tibble(feature,
                range_normed[1],
                range_normed[2],
                range_scaled[1],
                range_scaled[2]))
...
# Alternative: 
# Use LayerData() to isolate each layer
data_layer  <- LayerData(pre_sobj, "data")
scale_layer <- LayerData(pre_sobj, "scale.data")

# Randomly select features from the scale data layer
# scale.data by default includes a subset of all features in data layer (2000 most variable)
# set.seed(111) # could run for reproducibility
features <- sample(rownames(scale_mat), 10)

# compare the ranges for each feature
range_df <- data.frame(
  data_min = apply(data_mat[features, , drop = FALSE], 1, min),
  data_max = apply(data_mat[features, , drop = FALSE], 1, max),
  scale_min = apply(scale_mat[features, , drop = FALSE], 1, min),
  scale_max = apply(scale_mat[features, , drop = FALSE], 1, max)
)
...
```{r}
summary.seurat <- function(obj) {
  # Number of cells
  cat("Cells:", length(Cells(obj)), "\n")
  # Number of features (from default assay)
  cat("Features:", length(Features(obj)), "\n")
  # Active identities
  cat("Active identities:\n")
  print(unique(Idents(obj)))
  # Default assay
  cat("Default assay:", DefaultAssay(obj), "\n")
  # Plot if reductions exist
  if (length(Reductions(obj)) > 0) {
    print(DimPlot(obj))
  }
}

summary.seurat(pre_sobj)
```
