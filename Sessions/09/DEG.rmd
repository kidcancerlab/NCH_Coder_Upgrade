...
### Read in T-cell data, set assay to the RNA slot, and set idents to cell types:
```{r}
```
...
for (cell in unique(data$seurat_annotations)) {
    ...
    # get average expression for each gene for each sample ID across all cells
    df <- AverageExpression(sub,
            assays = "RNA",
            slot = "counts") %>%
            as.data.frame()
...
Note: if we are short on time just load in pseudobulk data provided.
...
cool_ref <- load(coolReference.RData)
ref <- cool_ref
