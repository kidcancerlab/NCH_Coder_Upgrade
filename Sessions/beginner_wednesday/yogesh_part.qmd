...
mm_mets <- qs::qread("/gpfs0/home2/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/output/seurat_objects/final_tumor_vs_stroma/mm_prim_normal_cells.qs")
mm_mets$nUMI <- mm_mets$nCount_RNA
mm_mets$batch <- mm_mets$model
...
corr_counts <-
    crescendo(
        Ycounts = GetAssayData(mm_mets, layer = 'counts'), # genes by cells
        meta_data = mm_mets@meta.data,                     # Cells by metadata
        R = t(Embeddings(mm_mets, reduction = "harmony")), # dims by cells
        batch_var = "batch",
        #genes_use = genes_use,
        #prop = 0.25,
        #min_cells = min_cells,
