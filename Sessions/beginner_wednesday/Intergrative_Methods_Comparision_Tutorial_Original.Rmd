...
InstallData("pbmcref")
InstallData("pbmcsca.SeuratData")
data("pbmcref")
data("pbmcsca")

# Here you can double check the available datasets in the Seurat sharing dataset for your own taining.
AvailableData()
```
...
# check the obj
obj
# check the mata datasets
obj@meta.data %>% head()

# check the levels of the methods
levels(as.factor(obj@meta.data$Method))

# we filtered the obj that have number of feature RNA less than 1000
obj  <- subset(obj, nFeature_RNA > 1000)
obj <- RunAzimuth(obj, reference = "pbmcref")
obj<-JoinLayers(obj)
# currently, the object has two layers in the RNA assay: counts, and data
saveRDS(object = obj, file = "IntegrativeMethods.RDS")
```
...
#TFBSTools::colSums(obj)
# change the version to  Matrix 1.6-3
# my current version of matrix is , we need to change to 1.6-1.1 for Seurat V5. remotes::install_version("Matrix", version = "1.6-1.1")
#library(Matrix)
...
obj <- IntegrateLayers(
  ...
  verbose = TRUE)
# Note: you might get ~9 Warning messages, The team is still working on fixing this so the warnings wont'c come up, but this should not affect your results.
