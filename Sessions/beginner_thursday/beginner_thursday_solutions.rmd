...
DimPlot(object = data,
        group.by = c("anchor_cell_types", "sex", "mouse"))
```
...
### Find genes that are differentially expressed between fibroblasts between female and male mice
...
write.csv(x = degs, 
          file = "fibroblasts_female_vs_male_degs.csv")
...
### Create a pseudobulk dataset for *anchor_cell_types*
```{r}
# create pseudobulked data for clusters across condition
pb_data <- 
  AggregateExpression(object = data, 
...
pb_data$cell_type_sex <- paste(pb_data$anchor_cell_types,
                                   pb_data$sex,
                                   sep = "_")
pb_data$cell_type_sex %>%
  head()
```

### Find differentially expressed genes for fibroblasts using the pseudobulk method
```{r}
# set default assay to RNA
DefaultAssay(pb_data) <- "RNA"

# perform DE testing on pseudbulked data for same cell type as above
Idents(pb_data) <- "cell_type_sex"
pseudobulked_degs <- 
  FindMarkers(object = pb_data, 
              ident.1 = "fibroblast_female",
              ident.2 = "fibroblast_male",
              test.use = "DESeq2")
pseudobulked_degs %>%
  head(n = 10)
...
Perform GSEA on the genes that are differentially expressed in the macrophages between female and male mice

### Find genes differentially expressed in macrophages between female and male mice
```{r}
Idents(data) <- "anchor_cell_types"
degs <- 
  FindMarkers(object = data,
...
```{r}
write.csv(degs,
          "macrophages_female_vs_male_degs.csv")
```
...
# save results as a .csv file (use fwrite to handle leadingEdge list)
fwrite(fgsea_res, "macrophage_female_vs_male_GSEA.csv")
```
