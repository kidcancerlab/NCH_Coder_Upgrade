...
## Activity
We can further look deeper into these annotation. As a part of this exercise we
will look at the immune cell types. We will subset the microglia and reprocess and
annoate in the deeper level using the markers given below.
Case: CD74 is expressed in microglia in the central nervous system, particularly in 
reactive microglia, which are activated microglia under disease conditions. Identify the 
cluster of cells within microglia that express Cd74 and annotate them as "Reactive Microglia".
leave all the options as default including the resolution.
```{r activity3}
