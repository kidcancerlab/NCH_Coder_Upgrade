...
```{r load}
# Load mouse kidney p0 male #5 scRNAseq dataset
## You will need to change the path below to use your NCH username 
## (or other path to desktop if not on an NCH computer)
m5_data <- Read10X(data.dir = "/igm/projects/scrgot/2026_data/GSE232482/GSM7337034_mouse_scRNA-seq_P0_M5/filtered_feature_bc_matrix")
```

# Create Seurat Object
```{r createobj}
m5 <- CreateSeuratObject(counts = m5_data,
                         project = "GSM7337034_mouse_scRNA-seq_P0_M5",
                         min.cells = 3,
                         min.features = 200)
...
```{r qc}
#### Visualize Outliers ####
m5[["percent.mt"]] <- PercentageFeatureSet(m5, pattern = "^mt-")
m5[["percent.ribo"]] <- PercentageFeatureSet(m5, pattern = "^Rp[sl]")
m5[["percent.hb"]] <- PercentageFeatureSet(m5, pattern = "^Hb[ab]")
...
plot1 <- FeatureScatter(m5,
                        feature1 = "nCount_RNA",
                        feature2 = "percent.mt") + 
  theme(legend.position="none")

plot2 <- FeatureScatter(m5,
                        feature1 = "nCount_RNA",
...
#### Run PCA ####
m5 <- RunPCA(m5,
             features = VariableFeatures(object = m5))
...
# Save
save(m5,
     file = "/igm/home/cnh008/projects/scrgot/scrgot_2026/workshop_data/rdata/m5.RData")

# saveRDS
saveRDS(m5,
        file = "/igm/home/cnh008/projects/scrgot/scrgot_2026/workshop_data/rdata/m5.rds")
```
...
```{r seuratload}
# Load
load("/igm/home/cnh008/projects/scrgot/scrgot_2026/workshop_data/rdata/m5.RData")

# readRDS
m5 <- readRDS("/igm/home/cnh008/projects/scrgot/scrgot_2026/workshop_data/rdata/m5.rds")
```

# Challenge:
Perform single cell analysis of the GSM7337031_mouse_scRNA-seq_P0_F2 dataset (10x): 

## Load in F2 dataset
```{r challenge} 
f2_raw <- Read10X(data.dir = "/igm/projects/scrgot/2026_data/GSE232482/GSM7337031_mouse_scRNA-seq_P0_F2/filtered_feature_bc_matrix") 
```

## Create Seurat Object
```{r challenge-obj} 
f2 <- CreateSeuratObject(counts = f2_raw,
                         project = "GSM7337031_mouse_scRNA-seq_P0_F2",
                         min.cells = 3,
                         min.features = 200)
...
f2[["percent.ribo"]] <- PercentageFeatureSet(f2, pattern = "^Rp[sl]")
f2[["percent.hb"]] <- PercentageFeatureSet(f2, pattern = "^Hb[ab]")
f2 <- subset(f2,
               subset = nFeature_RNA > 500 &
                 nFeature_RNA < 7000 &
                 nCount_RNA < 20000 &
...
```{r challenge-scale} 
f2 <- ScaleData(f2,
                features = rownames(f2))
```

## Run PCA
```{r challenge-pca} 
f2 <- RunPCA(f2,
             features = VariableFeatures(object = f2))
```
...
```{r challenge-save} 
saveRDS(f2,
        file = "/igm/home/cnh008/projects/scrgot/scrgot_2026/workshop_data/rdata/f2.rds")
```
