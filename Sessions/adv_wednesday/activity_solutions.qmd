...
```{r}
sender_celltypes_2 <-
  discard(keep_cell_types, keep_cell_types %in% activity_cell_type)
...
receiver_dotplot_2 <-
  kidney_data %>%
  subset(cell_type %in% activity_cell_type) %>%
  DotPlot(
    features = rownames(vis_ligand_receptor_network_2),
    scale = FALSE,
    group.by = "age"
  ) +
  labs(title = activity_cell_type) +
  theme(
    axis.text.x = element_text(angle = 90, vjust = 0.5),
