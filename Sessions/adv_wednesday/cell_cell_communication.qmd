...
```{r}
kidney_data <-
    qs::qread("workshop_data/rdata/merged_sobj_harmony_sample_batch_theta.qs")
...
de_list <-
    parallel::mclapply(
        ...
        function(this_cell_type) {
            ...
            design <-
                data.frame(
                    sex = ifelse(str_detect(pattern = "F", string = colnames(pseudobulked_data)),"F","M") %>%
                        as.factor(),
...
# add in sex information since it doesn't look like it's in there yet...
kidney_data$sex <-
  ifelse(str_detect(pattern = "F", string = kidney_data$orig.ident),"female","male")

# Lets count how many cells are in each cell type
table(kidney_data$anchor_cell_types, kidney_data$sex) %>%
    as.data.frame() %>%
    filter(Var1 %in% keep_cell_types) %>%
    dplyr::rename(anchor_cell_types = Var1) %>%
    pivot_wider(
        names_from = "Var2",
        values_from = "Freq"
    ) %>%
...
```{r}
Idents(kidney_data) <- kidney_data$cell_type
## receiver
