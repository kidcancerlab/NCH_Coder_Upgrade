...
Two main types of objects in R
...
- Can exend base R functions to work with objects of a certain type
...
```
What data are in the rownames of the metadata?
...
```{r}
my_data <- GetAssayData(pbmc_small, assay = "RNA", slot = "counts")
...
```{r}
ave_expr <- AverageExpression(pbmc_small, group.by = "letter.idents")$RNA
ave_expr[1:5, ]
```
...
```{r}
pbmc_small$nonsense <- sample(LETTERS[1:5],
                              ncol(pbmc_small),
                              replace = TRUE)
...
```{r}
high_cd79b_data <-
    FetchData(pbmc_small, vars = rownames(pbmc_small), cells = high_cd79b)
